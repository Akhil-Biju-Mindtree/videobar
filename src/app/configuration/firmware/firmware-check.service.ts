import { Injectable, NgZone } from '@angular/core';
import { Observable, Subject, BehaviorSubject } from 'rxjs';
import { ConnectionDetectionService } from '@core/services/connection-detection.service';
import { DeviceDataManagerService } from '@core/services/device-data-manager.service';
import { CONFIGURATION_CONSTANTS } from '../configuration.constant';
import { FirmwareConstants } from './firmware.constant';
import { takeUntil } from 'rxjs/operators';
import { AppConstants } from '@core/constants/app.constant';
import { Logger } from '@core/logger/Logger';
import { HttpClientWrapperService } from '@core/services/http-client-wrapper.service';

@Injectable({
  providedIn: 'root',
})
export class FirmwareCheckService {
  unsubscribe = new Subject<void>();
  currentFirmwareVersion;
  latestFirmwareVersion;
  noConnection = true;
  private isUpToDate: BehaviorSubject<Object> = new BehaviorSubject<Object>({
    isUpToDate: true,
    noConnection: true,
    details: null,
  });
  constructor(
    private deviceManagerService: DeviceDataManagerService,
    private connectionDetectionService: ConnectionDetectionService,
    private http: HttpClientWrapperService,
    private ngZone: NgZone,
    private loggerService: Logger,
  ) {
    deviceManagerService
      .listenFromDevice(FirmwareConstants.UUID.FIRMWARE_VERSION)
      .pipe(takeUntil(this.unsubscribe))
      .subscribe((currentFirmwareVersion: string) => {
        ngZone.run(() => {
          if (currentFirmwareVersion) {
            this.currentFirmwareVersion = currentFirmwareVersion;
            if (navigator.onLine) {
              this.noConnection = false;
              this.checkFirmware();
            } else {
              this.noConnection = true;
              this.isUpToDate.next({ isUpToDate: true, noConnection: true, details: null });
            }
            this.checkForConnection();
          }
        });
      });
  }

  private checkForConnection() {
    this.connectionDetectionService.getConnectionStatus().subscribe((connectionStatus: string) => {
      this.ngZone.run(() => {
        if (connectionStatus) {
          if (connectionStatus === AppConstants.ONLINE) {
            this.noConnection = false;
            this.checkFirmware();
          } else {
            this.noConnection = true;
            this.isUpToDate.next({ isUpToDate: true, noConnection: true, details: null });
          }
        }
      });
    });
  }

  public checkFirmware() {
    this.http.fetchVersionedLink(FirmwareConstants.FIRMWARE_INFO_URL).subscribe(
      (firm_info: any) => {
        this.ngZone.run(() => {
          this.latestFirmwareVersion = firm_info.requestVersion;
          const fwData = { ...firm_info.FW, latestFirmwareVersion: this.latestFirmwareVersion };
          if (this.compareVersion(this.currentFirmwareVersion, this.latestFirmwareVersion) < 0) {
            this.isUpToDate.next({ isUpToDate: false, noConnection: false, details: fwData });
          } else {
            this.isUpToDate.next({ isUpToDate: true, noConnection: false, details: fwData });
          }
        });
      },
      (error: any) => {
        this.loggerService.trace(`Could not connect to server::: ${JSON.stringify(error)}`);
      },
    );
  }

  compareVersion(v1, v2) {
    if (typeof v1 !== 'string') {
      return false;
    }
    if (typeof v2 !== 'string') {
      return false;
    }
    const v1parts = v1.split('.');
    const v2parts = v2.split('.');
    const k = Math.min(v1parts.length, v2parts.length);
    for (let i = 0; i < k; i = i + 1) {
      const v1n = parseInt(v1parts[i], 10);
      const v2n = parseInt(v2parts[i], 10);
      if (v1n > v2n) {
        return 1;
      }
      if (v1n < v2n) {
        return -1;
      }
    }
    return v1.length === v2.length ? 0 : v1.length < v2.length ? -1 : 1;
  }

  isFirmwareUpToDate(): Observable<Object> {
    return this.isUpToDate.asObservable();
  }
}
